...
### Importing the credit card dataset from Kaggle
```{r}
...
From the experiment, upsampling peformed slightly better than ROSE and Down Sampling. However, we will use Down Sampling to reduce the time for model training and execution.
...
We can also check which variables has signigicant role in fraud detection.
```{r}
...
From the above plots and models, we can clarify that XGBoost performed better than logistic and Random Forest Model, although the margin was not very high.
